...
export class CatDoorSwitch extends Device {
	constructor(name, open_switch, close_switch, delay, photointerruptor, powermonitor, field, limit) {
		super(name);

		// The devices that open and close the door, and the delay required during reversal

		this.open_switch = open_switch;
		this.close_switch = close_switch;
		this.delay = delay;

		// The device that indicates that the door isn't safe to close

		this.photointerruptor = photointerruptor;

		// The power monitor indicating whether the door is trying to crush something
		// and the field that Device shows the wattage, and the limit for that field

		this.powermonitor = powermonitor;
		this.field = field;
		this.limit = limit;

		// We also have a timeout that indicates when the power monitor should be disabled

		this.powermonitor_timeout_id = null;

		this.on('change_power', (new_value) => this.power_changed(new_value));

		powermonitor.on('change_' + field, (new_value) => this.wattage_changed(new_value));

		photointerruptor.on('change_power', (new_value) => this.photointerruptor_changed(new_value));
	}

	// When we toggle this device's power on or off, it translates to turning on the opener or
	// closer (which is usually a single piston).  Also, there is a delay during reversal (mostly so that
	// the piston motor's momentum doesn't force power backwards through the power supply).

	power_changed(new_value) {
		if (new_value) {
			this.close_switch.power(0);
			setTimeout(() => this.open_switch.power(1), this.delay).unref();
		}
		else if (!this.photointerruptor.state().power) {
			this.enable_powermonitor_temporarily(30000);
			this.open_switch.power(0);
			setTimeout(() => this.close_switch.power(1), this.delay).unref();
		}
		else {
			debug("Can't close door-- photointerruptor is blocked");
		}
	}

	// We turn on this HTTP polling power monitor just for the time that the door is actually in motion
	// and being closed.

	enable_powermonitor_temporarily(timeout) {
		if (this.powermonitor_timeout_id) {
			clearTimeout(this.powermonitor_timeout_id);
		}

		this.powermonitor_timeout_id = setTimeout(() => this.powermonitor.modify({ enable: false }), timeout).unref();

		this.powermonitor.modify({ enable: true });
	}

	// If the wattage exceeds a certain value, we open the door so it won't crush whatever is
	// presumably blocking it.

	wattage_changed(new_value) {
		if (new_value > this.limit) {
			debug("Over-power condition for", this.name);
			this.power(true);
		}
	}

	// If the photointerruptor is triggered during door close, the door opens again.

	photointerruptor_changed(new_value) {
		if (new_value && this.close_switch.state().power) {
			debug("Photo Interruptor triggered for", this.name);
			this.power(true);
		}
	}
}
