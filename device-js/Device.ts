...
const debug = logger("homeserver:device");
...
import { StateHolder, StatePublisher } from "@homeserver-js/core";
...
export class Device extends EventEmitter {
  ...
  with(...items: Transceiver[]) {
    items.forEach((item) => {
      if (item instanceof Transmitter) {
        return this.add_transmitter(item);
      }
      if (item instanceof Receiver) {
        return this.add_receiver(item);
      }
      if (item instanceof Configurator) {
  ...
  receive(receiver: Receiver, state: DeviceState) {
    if (receiver._prevent_events) {
      debug(
        "Modifying state for",
        this.name,
        "without emitting events because of prevent_events",
      );
...
export class PrioritizedSwitch extends Switch {
  ...
  power_priority(value: boolean, my_priority: number, timeout: number = 0) {
    ...
    if (
      ...
      this.state().priority <= my_priority
    ) {
      this.modify({
      ...
      if (timeout) {
        this.timeout_id = setTimeout(() => {
          this.clear_priority(my_priority);
        }, timeout).unref();
      }
    }
  }
  ...
  constructor(name: string, ...devices: Device[]) {
    ...
    this.on("stateChange", (field, new_value) => {
      this.devices.forEach((device) => {
        debug_composite(`Modifying ${device.name} with `, {
          [field]: new_value,
        });
...
export class Shelly1PMPolling extends Device {
  ...
  constructor(
    ...
    this.period = period;
    this.pollable_receiver = new HTTPGetPollParsed(
      { url: url },
      0,
      "watts",
      (data) => this.parse_power(data),
    );
...
export class PowerMonitoredSwitch extends Device {
  constructor(
    ...
    this.on("change_power", (new_value) =>
      this.subdevice.modify({ power: new_value }),
    );
  ...
  constructor(
    ...
    this.subdevice.on("change_power", () => {
      ...
      if (!this.next_change_is_automatic) {
        ...
        debug_automatic(
          this.name,
          " power change forwarded from AutomaticSwitch",
        );
        this.next_change_is_automatic = false;
      }
    });
    ...
    this.on("change_power", (new_value) => {
      if (this.subdevice_last_commanded_change + this.timeout < Date.now()) {
        debug_automatic(
          this.name,
          " Timeout reached; forwarding power change request",
        );
        ...
        debug_automatic(
          this.name,
          " Ignoring power change request, timeout left =",
...
export class AutoOffSwitch extends Device {
  ...
  timer_on() {
    ...
    this.subdevice.once("change_" + this.field, (new_value, old_value) =>
      this.subdevice_changed(),
    );
  }
...
export class DelayedOnSwitch extends Device {
  timeout_id: NodeJS.Timeout | null = null;
